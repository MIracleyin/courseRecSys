...
class GeneralGraphDataset(RecBoleDatase):
    ...
    def get_sgl_norm_adj_mat(self):
        ...
        row = self.inter_feat[self.iid_field]
        col = self.inter_feat[self.cid_field] + self.item_num

        edge_index1 = torch.stack([row, col])
        edge_index2 = torch.stack([col, row])
        edge_index = torch.cat([edge_index1, edge_index2], dim=1)

        deg = degree(edge_index[0], self.item_num + self.cate_num)
